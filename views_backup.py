...
def validate_folder_path(request):
    ...
    try:
        ...
        if folder_type == 'images':
            ...
            if file_count == 0:
                return JsonResponse({'valid': False, 'message': 'No image files found in directory'})
        elif folder_type == 'json':
            json_files = [f for f in files if f.lower().endswith('.json')]
            file_count = len(json_files)
            if file_count == 0:
                return JsonResponse({'valid': False, 'message': 'No JSON files found in directory'})
        else:
        ...
        return JsonResponse({'valid': False, 'message': f'Error validating path: {str(e)}'})
