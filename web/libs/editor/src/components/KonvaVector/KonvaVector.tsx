...
export const KonvaVector = forwardRef<KonvaVectorRef, KonvaVectorProps>((props, ref) => {
	...
	const [initialPoints, setInitialPoints] = useState(() =>
		normalizePoints(rawInitialPoints)
	);
	...
	const [_newPointDragIndex, setNewPointDragIndex] = useState<number | null>(
		null,
	);
	...
	const [visibleControlPoints, setVisibleControlPoints] = useState<Set<number>>(
		new Set(),
	);
	...
	const isDrawingDisabled = () => {
				...
				if (point.isBezier) {
					...
					if (point.controlPoint1) {
						const distance = Math.sqrt(
							(cursorPosition.x - point.controlPoint1.x) ** 2 +
							(cursorPosition.y - point.controlPoint1.y) ** 2,
						);
					...
					if (point.controlPoint2) {
						const distance = Math.sqrt(
							(cursorPosition.x - point.controlPoint2.x) ** 2 +
							(cursorPosition.y - point.controlPoint2.y) ** 2,
						);
						if (distance <= controlPointHitRadius) {
							return true; // Disable drawing when hovering over control points
						}
		...
		if (cursorPosition && initialPoints.length > 0) {
			...
			for (let i = 0; i < initialPoints.length; i++) {
				const point = initialPoints[i];
				const distance = Math.sqrt(
					(cursorPosition.x - point.x) ** 2 + (cursorPosition.y - point.y) ** 2,
				);
	...
	const convertPointHandler = (pointIndex: number) => {
		...
		if (point.isBezier) {
			...
			if (
				pointInfo.pathIndex === 0 ||
				pointInfo.pathIndex === initialPoints.length - 1
			) {
				return;
			}
	...
	useImperativeHandle(ref, () => ({
		...
		exportShape: () => {
			...
			// Check if we have enough points based on minPoints constraint
			const incomplete =
				minPoints !== undefined && initialPoints.length < minPoints;
	...
	return (
		<Layer
			...
			onMouseDown={eventHandlers.handleLayerMouseDown}
			onMouseMove={eventHandlers.handleLayerMouseMove}
			onMouseUp={eventHandlers.handleLayerMouseUp}
			onClick={eventHandlers.handleLayerClick}
			onDblClick={eventHandlers.handleLayerDblClick}
		>
