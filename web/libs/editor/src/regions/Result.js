...
const Result = types
  .model("Result", {
    ...
    type: ff.isActive(ff.FF_CUSTOM_TAGS)
      ? types.late(() => types.enumeration([...resultTypes, ...Registry.customTags.filter((t) => t.resultName).map((t) => t.resultName)]))
    ...
    value: ff.isActive(ff.FF_CUSTOM_TAGS)
      ? types.late(() =>
          types.model({
            ...resultValues,
            ...Object.fromEntries(Registry.customTags.filter((t) => t.resultName).map((t) => [t.resultName, types.maybe(t.result)])),
          }),
        )
      : types.model({
          ...resultValues,
