...
const Model = types
  ...
  .views((self) => ({
    ...
    get dataObj() {
      if (!isAlive(self) || !self.valueLoaded || !self.data) return null;
      let data = self.data;
    ...
    formatTime(time) {
      if (!isAlive(self)) return String(time);
      if (!self._format) {
    ...
    formatDuration(duration) {
      if (!isAlive(self)) return String(duration);
  ...
  .actions((self) => ({
    ...
    _handlePause(data) { // data.time is received in relative seconds
      if (!isAlive(self)) {
    ...
    playbackLoop() {
      ...
      if (targetNativeForUpdate >= maxKey) {
        ...
        self.playStartTime = null;
        self.playStartPosition = null;
        if (self.animationFrameId) { cancelAnimationFrame(self.animationFrameId); self.animationFrameId = null; }
    ...
    _updateViewForTime(time) {
      ...
      if (!isFinite(newTimeStartNative) || !isFinite(newTimeEndNative) || newTimeStartNative >= newTimeEndNative) {
        ...
        if (!isFinite(newTimeStartNative) || !isFinite(newTimeEndNative) || newTimeStartNative >= newTimeEndNative) return;
      }
...
const Overview = observer(({ item, data, series }) => {
  ...
  function brushstarted() {
    ...
    if (eventSelection && !checkD3EventLoop("brush") && !checkD3EventLoop("wheel")) {
      let [x1, x2] = eventSelection;
  ...
  function brushended() {
    if (!isAlive(item)) return;
    if (!d3.event.selection && gb.current) {
  ...
  React.useEffect(() => {
    if (node && isAlive(item) && gChannels.current && gb.current) {
      d3.select(node)
        .selectAll("svg")
        .attr("viewBox", [0, 0, width + margin.left + margin.right, focusHeight + margin.bottom]);
  ...
  React.useEffect(() => {
    if (!gb.current || !isAlive(item)) return;
    const range = item.brushRange.map(x);
...
const HtxTimeSeriesViewRTS = ({ item }) => {
  ...
  if (!isAlive(item) || !item?.brushRange?.length || !item.data)
    return (
