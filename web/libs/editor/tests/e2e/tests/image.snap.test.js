...
const isSnappedToPixel = (coord, imageSize, expectedCoord) => {
  ...
  console.log(
    `Checking snapped coordinate ${coord}% on image size ${imageSize}: pixel coord = ${pixelCoord}, expected coord = ${expectedCoord}, diff = ${Math.abs(pixelCoord - expectedCoord)}`,
  );
...
Scenario(
  "RectangleLabels snap to pixel during drawing",
  async ({ I, LabelStudio, AtImageView, AtOutliner, AtLabels, AtPanels }) => {
    const AtDetailsPanel = AtPanels.usePanel(AtPanels.PANEL.DETAILS);
    ...
    AtImageView.drawByDrag(
      drawCoords.x,
      drawCoords.y,
      drawCoords.x + drawCoords.width,
      drawCoords.y + drawCoords.height,
    );
    AtOutliner.seeRegions(1);
