...
from .models import ModelInterface, ThirdPartyModelVersion, ModelRun
...
class ModelRunAdmin(admin.ModelAdmin):
    ...
    def delete_model_run_predictions(self, request, queryset):
        ...
        self.message_user(
            request, f"Deleted predictions for {queryset.count()} model runs."
        )

    delete_model_run_predictions.short_description = (
        "Delete predictions for selected model runs"
    )
