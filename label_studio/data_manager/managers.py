...
# Tests for excluded_fields_for_evaluation functionality
def test_annotate_queryset_excluded_fields():
    """Test that annotate_queryset properly excludes fields when excluded_fields_for_evaluation is provided."""
    print("🧪 Testing annotate_queryset with excluded_fields_for_evaluation...")
    
    # Mock queryset and dependencies
    class MockTask:
        project = None
    ...
    # Mock annotation functions
    def mock_expensive_function(queryset):
        queryset.annotations_applied.append('expensive_field')
        return queryset
    
    def mock_cheap_function(queryset):
        queryset.annotations_applied.append('cheap_field')
        return queryset
    
    # Test setup
    original_map = get_annotations_map()
    test_map = {
        'expensive_field': mock_expensive_function,
        'cheap_field': mock_cheap_function,
        'another_expensive_field': mock_expensive_function,
    }
    
    try:
        # Temporarily replace annotations map  
        update_annotation_map(test_map)
        
        queryset = MockQuerySet()
        
        # Test 1: all_fields=True without exclusions (should apply all)
        result = PreparedTaskManager.annotate_queryset(
            queryset, 
            all_fields=True
        )
        expected_all = ['expensive_field', 'cheap_field', 'another_expensive_field']
        assert set(result.annotations_applied) == set(expected_all), f"Expected {expected_all}, got {result.annotations_applied}"
        print("  ✅ Test 1 passed: all_fields=True applies all annotations")
        
        # Test 2: all_fields=True with exclusions (should exclude specified fields)
        queryset = MockQuerySet()
        result = PreparedTaskManager.annotate_queryset(
            queryset,
            all_fields=True,
            excluded_fields_for_evaluation=['expensive_field', 'another_expensive_field']
        )
        expected_excluded = ['cheap_field']
        assert result.annotations_applied == expected_excluded, f"Expected {expected_excluded}, got {result.annotations_applied}"
        print("  ✅ Test 2 passed: excluded_fields_for_evaluation properly excludes fields")
        
        # Test 3: fields_for_evaluation with exclusions (exclusions should override)
        queryset = MockQuerySet()
        result = PreparedTaskManager.annotate_queryset(
            queryset,
            fields_for_evaluation=['expensive_field', 'cheap_field'],
            excluded_fields_for_evaluation=['expensive_field']
        )
        expected_selective = ['cheap_field']
        assert result.annotations_applied == expected_selective, f"Expected {expected_selective}, got {result.annotations_applied}"
        print("  ✅ Test 3 passed: exclusions override fields_for_evaluation")
        
        # Test 4: empty exclusions list (should behave like no exclusions)
        queryset = MockQuerySet()
        result = PreparedTaskManager.annotate_queryset(
            queryset,
            all_fields=True,
        ...
        expected_empty_exclusions = ['expensive_field', 'cheap_field', 'another_expensive_field']
        assert set(result.annotations_applied) == set(expected_empty_exclusions), f"Expected {expected_empty_exclusions}, got {result.annotations_applied}"
        print("  ✅ Test 4 passed: empty exclusions list works correctly")
        
        print("🎉 All annotate_queryset tests passed!")
        
    finally:
        # Restore original annotations map
        managers_module.settings.DATA_MANAGER_ANNOTATIONS_MAP = original_map


def test_get_queryset_excluded_fields():
    """Test that get_queryset properly passes excluded_fields_for_evaluation to annotate_queryset."""
    print("\n🧪 Testing get_queryset with excluded_fields_for_evaluation...")
    
    # Mock the annotate_queryset method to capture parameters
    class MockPreparedTaskManager(PreparedTaskManager):
        def __init__(self):
            self.annotate_queryset_calls = []
        
        def only_filtered(self, prepare_params=None):
            # Return mock queryset
            class MockFilteredQueryset:
                pass
            return MockFilteredQueryset()
        
        def annotate_queryset(self, queryset, fields_for_evaluation=None, all_fields=False, excluded_fields_for_evaluation=None, request=None):
            # Record the call parameters
            self.annotate_queryset_calls.append({
                'fields_for_evaluation': fields_for_evaluation,
                'all_fields': all_fields,
                'excluded_fields_for_evaluation': excluded_fields_for_evaluation,
                'request': request
    ...
    manager = MockPreparedTaskManager()
    prepare_params = MockPrepareParams()
    
    # Test 1: get_queryset with excluded_fields_for_evaluation
    manager.get_queryset(
        fields_for_evaluation=['field1', 'field2'],
        prepare_params=prepare_params,
        all_fields=True,
        excluded_fields_for_evaluation=['expensive_field1', 'expensive_field2']
    )
    
    # Verify the call was made with correct parameters
    assert len(manager.annotate_queryset_calls) == 1, "annotate_queryset should be called once"
    call = manager.annotate_queryset_calls[0]
    
    assert call['fields_for_evaluation'] == ['field1', 'field2'], f"fields_for_evaluation mismatch: {call['fields_for_evaluation']}"
    assert call['all_fields'] == True, f"all_fields mismatch: {call['all_fields']}"
    assert call['excluded_fields_for_evaluation'] == ['expensive_field1', 'expensive_field2'], f"excluded_fields_for_evaluation mismatch: {call['excluded_fields_for_evaluation']}"
    assert call['request'] == 'mock_request', f"request mismatch: {call['request']}"
    
    print("  ✅ Test 1 passed: get_queryset correctly passes excluded_fields_for_evaluation")
    
    # Test 2: get_queryset without excluded_fields_for_evaluation (should default to None)
    manager.annotate_queryset_calls = []  # Reset
    manager.get_queryset(
        fields_for_evaluation=['field1'],
        prepare_params=prepare_params,
        all_fields=False
        # excluded_fields_for_evaluation not provided
    )
    
    call = manager.annotate_queryset_calls[0]
    assert call['excluded_fields_for_evaluation'] is None, f"excluded_fields_for_evaluation should default to None: {call['excluded_fields_for_evaluation']}"
    
    print("  ✅ Test 2 passed: get_queryset defaults excluded_fields_for_evaluation to None")
    
    print("🎉 All get_queryset tests passed!")


def run_excluded_fields_tests():
    """Run all tests for excluded_fields_for_evaluation functionality."""
    print("🔬 RUNNING EXCLUDED_FIELDS_FOR_EVALUATION TESTS")
    print("=" * 60)
    
    try:
        test_annotate_queryset_excluded_fields()
        test_get_queryset_excluded_fields()
        
        print("\n✅ ALL TESTS PASSED! excluded_fields_for_evaluation is working correctly.")
        print("🚀 Ready for production deployment!")
        
    except Exception as e:
        print(f"\n❌ TEST FAILED: {str(e)}")
        print("⚠️  Fix the issue before deploying!")
        raise


# Uncomment the line below to run tests when this file is imported
# run_excluded_fields_tests()
